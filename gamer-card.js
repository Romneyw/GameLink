...
$('reset-draft').addEventListener('click',()=>{if(!confirm('Start a new card? This replaces the draft on this device.'))return;imageVersion++;backgroundVersion++;backgroundImage=null;backgroundData=null;$('background-status').textContent='';$('card-form').reset();$('timezone').value=Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC';avatar=null;avatarData=null;$('avatar-controls').hidden=true;changed();});
async function init(){
