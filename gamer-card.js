...
const fields=['gamer-name','platform','language','player-id','play-times','timezone','play-style','microphone','zoom','position-x','position-y'];
...
function text(str,x,y,size=28,color='#eff0f7',weight='400',width=900){ctx.font=weight+' '+size+'px Arial';ctx.fillStyle=color;while(ctx.measureText(str).width>width&&size>15){size--;ctx.font=weight+' '+size+'px Arial';}if(ctx.measureText(str).width>width){while(str.length&&ctx.measureText(str+'…').width>width)str=str.slice(0,-1);str+='…';}ctx.fillText(str,x,y);}
...
function render(){
 const d=design(),accent={crimson:'#ff4775',ion:'#47dfff',venom:'#b8ff45'}[d];ctx.clearRect(0,0,1080,1440);
 rect(0,0,1080,1440,42,'#090c14');const glow=ctx.createRadialGradient(d==='crimson'?930:540,250,20,540,400,760);glow.addColorStop(0,accent+'35');glow.addColorStop(1,'#090c1400');ctx.fillStyle=glow;ctx.fillRect(0,0,1080,1440);
 ...
 ctx.save();ctx.strokeStyle=accent+'20';ctx.lineWidth=1;if(d==='venom'){for(let x=60;x<1040;x+=40){ctx.beginPath();ctx.moveTo(x,120);ctx.lineTo(x,680);ctx.stroke();}for(let y=120;y<700;y+=40){ctx.beginPath();ctx.moveTo(60,y);ctx.lineTo(1020,y);ctx.stroke();}}else if(d==='crimson'){ctx.fillStyle=accent+'12';ctx.beginPath();ctx.moveTo(700,35);ctx.lineTo(1040,35);ctx.lineTo(650,660);ctx.lineTo(450,660);ctx.fill();}else{for(let r=245;r<380;r+=40){ctx.beginPath();ctx.arc(540,397,r,0,Math.PI*2);ctx.stroke();}}ctx.restore();
 ...
 ctx.fillStyle='#303948';ctx.fillRect(68,1330,944,1);text('GAMELINK / FIND YOUR PEOPLE',72,1370,20,accent,'700',600);text('Ranks are self-reported',710,1370,17,'#929caf','400',300);
 canvas.setAttribute('aria-label','Gamer card for '+value('gamer-name','your gamer name')+', '+d+' design. '+items.map(x=>x.join(': ')).join('. '));
}
function snapshot(){return{version:1,values:Object.fromEntries(fields.map(id=>[id,$(id).value])),design:design(),avatar:avatarData};}
...
$('card-form').addEventListener('submit',e=>e.preventDefault());$('card-form').addEventListener('input',e=>{if(e.target.id!=='avatar-file')changed();});$('card-form').addEventListener('change',e=>{if(e.target.id!=='avatar-file')changed();});
...
$('reset-draft').addEventListener('click',()=>{if(!confirm('Start a new card? This replaces the draft on this device.'))return;imageVersion++;$('card-form').reset();$('timezone').value=Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC';avatar=null;avatarData=null;$('avatar-controls').hidden=true;changed();});
async function init(){
 ...
 if(draft?.version===1){for(const id of fields){if(typeof draft.values?.[id]==='string')$(id).value=draft.values[id];}if(['crimson','ion','venom'].includes(draft.design))document.querySelector('input[value="'+draft.design+'"]').checked=true;if(draft.avatar){try{avatar=await decode(draft.avatar);avatarData=draft.avatar;$('avatar-controls').hidden=false;}catch{$('avatar-status').textContent='The saved avatar could not load. Please select it again.';}}$('draft-status').textContent='Your saved draft is restored.';}else $('draft-status').textContent='Your draft will be saved on this device as you edit.';
 }catch{$('draft-status').textContent='Device storage is unavailable. Keep this page open to retain your draft.';}
 finally{restoring=false;$('card-form').inert=false;$('reset-draft').disabled=false;render();}
}
void init();
})();
